...
contract ProposalAndFunding {
   ...
   constructor() {
    ...
    function removeAdmin(address _admin) public onlyAdmin {
        ...
        for (uint i = 0; i < adminList.length; i++) {
            if (adminList[i] == _admin) {
                adminList[i] = adminList[adminList.length.sub(1)];
                adminList.pop();
                break;
            }
        }
    ...
    function createProposalAndStartFunding(
        ...
        uint256 endTime = startTime.add(_durationInDays.mul(1 days)); // 지정된 기간 뒤에 종료될 시간 설정
        ...
        emit ProposalCreated(proposals.length.sub(1), msg.sender, _fundingGoal, startTime, endTime);
    }
    ...
    function fundProposal(uint256 _proposalId) public payable {
        ...
        require(proposal.amountRaised.add(msg.value) <= proposal.fundingGoal, "Funding goal exceeded");
    ...
    function updateContributions(uint256 _proposalId, address _contributor, uint256 _amount) internal {
        contributions[_proposalId][_contributor].amount = contributions[_proposalId][_contributor].amount.add(_amount);
        contributions[_proposalId][_contributor].timestamp = block.timestamp;
        proposals[_proposalId].amountRaised = proposals[_proposalId].amountRaised.add(_amount);
    ...
    function getSuccessfulFundingDetails() public view returns (
        ...
        // 100% 성공한 펀딩의 개수 세기
        for (uint256 i = 0; i < proposals.length; i++) {
            if (proposals[i].fundingClosed && proposals[i].amountRaised >= proposals[i].fundingGoal) {
                count++;
            }
        }
        ...
        totalFundingAmounts = new uint256[](count);
        contributors = new address[][](count);
        fundingAmounts = new uint256[][](count);
        fundingTimes = new uint256[](count);
        fundingShares = new uint256[](count);
        uint256 index = 0;
        ...
        for (uint256 i = 0; i < proposals.length; i++) {
            if (proposals[i].fundingClosed && proposals[i].amountRaised >= proposals[i].fundingGoal) {
                ...
                totalFundingAmounts[index] = proposals[i].amountRaised;

                // 기여자 정보 가져오기
                address[] memory contribs = proposalContributors[i];
                uint256[] memory amounts = new uint256[](contribs.length);
                for (uint256 j = 0; j < contribs.length; j++) {
                    amounts[j] = contributions[i][contribs[j]].amount;
                }

                contributors[index] = contribs;
                fundingAmounts[index] = amounts;
                fundingTimes[index] = proposals[i].endTime; // 펀딩 종료 시간으로 설정

                // 펀딩 지분 계산
                uint256 fundingShare = (proposals[i].amountRaised.mul(100)).div(proposals[i].fundingGoal);
                fundingShares[index] = fundingShare;

                index++;
            }
        }

        return (proposalIds, proposers, totalFundingAmounts, contributors, fundingAmounts, fundingTimes, fundingShares);
    }
}
